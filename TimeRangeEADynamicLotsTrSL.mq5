...
input bool stopLossTrailing = false;
...
// return if no stop loss or fixed stop loss
   if(stopLoss == 0 || !stopLossTrailing)
     {
      return;
     }
      ...
      if(magic_number == magicNB)
        {
         ...
         double newSL = NormalizeDouble(currPrice - ((range.high - range.low) * stopLoss * 0.01 * n), _Digits);
         ...
         if(!trade.PositionModify(ticket, newSL, currTP))
           {
            Print("Failed to modify position, ticket:", (string)ticket, " currSL:", (string)currSL, "newSL", (string)newSL, "currTP", (string)currTP);
            return;
           }
        }
     }
